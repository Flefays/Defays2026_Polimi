# -*- coding: utf-8 -*-
"""
Created on Mon Jun  1 13:11:29 2026
...
Description:
    Topological network of an Organic Rankine Cycle (ORC) power
    plant driven by high-enthalpy geothermal fluid.
    
    The thermodynamic variables free to be used without modifying this scripts are
    listed in "Free Specifications" below. Note it was designed in a certain context,
    it is relevant to modify the parameters ("Fixed Specifications") to your study case.
    
    The evaporators pinch (ttd_min) is not stated because the evaporation pressure is used 
    as optimisation variable. Fixing both p_evap & the pinch would lead to jacobian singularity. 
...
Fixed Specifications:
    - Pressure drops
    - condenser pinch
    - Preheater approach (fixed by x_r4)
...
from tespy.components import (
    ...
    PowerBus, PowerSink, Pump, Sink, Source, Turbine, DropletSeparator, Merge, Splitter, Drum
)
...
class HighEnthalpyGeoORC(ModelTemplate):
    ...
    def __init__(self, working_fluid, cooling_fluid, geofluidTemperature, geofluidFlow, geofluidVapour, evaporationPressure, T_amb, P_amb, dT_cd):
        self.working_fluid = working_fluid  # ex: ('Isopentane', 'R134a')
    ...
    def _parameter_lookup(self) -> dict:
        return {
            ...
            "evap_brine_pinch": ["Components", "geobrine evaporator", "ttd_min"],
    ...
    def _create_network(self) -> None:
        ...
        r4 = Connection(preheater, 'out2', drum, 'in1', label = 'r4')
        r5 = Connection(drum, 'out1', evap_splitter, 'in1', label = 'r5')
        r6 = Connection(evap_splitter, 'out1', evap_steam, 'in2', label = 'r6')
        r7 = Connection(evap_splitter, 'out2', evap_brine, 'in2', label = 'r7')
        r8 = Connection(evap_steam, 'out2', evap_merge, 'in1', label = 'r8')
        r9 = Connection(evap_brine, 'out2', evap_merge, 'in2', label = 'r9')
        r10 = Connection(evap_merge, 'out1', drum, 'in2', label = 'r10')
        r11 = Connection(drum, 'out2', orc_cc, 'in1', label = 'r11')
        ...
        self.nw.add_conns(f0, f1, f2, f3, f4, f5, f6, r0, r1, r2, r3, r4, r5, r6, r7, r8, r9, r10, r11, a0, a1, a2)
        ...
        # Saturation pressure for evaporators (Optimization Variable)
        r0.set_attr(p = self.p_evap, m0 = 50) # the drum run the x fraction
        ...
        evap_steam.set_attr()                      # pr2 fixed by the drum, splitter, merge
        evap_brine.set_attr(pr1 = 0.99)            # pr2 fixed by the drum, splitter, merge
